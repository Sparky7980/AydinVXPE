...
local vapeAssetTable = {
	...
	['rendervape/assets/BrainCircut.png'] = 'rbxassetid://13350877564'
}
...
for i,v in pairs({baseDirectory:gsub("/", ""), "vape", "rendervape/libraries", "rendervape/games", "rendervape/configuration", "rendervape/assets"}) do 
	if not isfolder(v) then makefolder(v) end
end

getgenv().rendervapeload = {file = 'GuiLibrary.lua', step = 2};
GuiLibrary = loadfile('rendervape/core/gui.lua')();
shared.rendervape = GuiLibrary;
...
local Matchmaking = GuiLibrary.CreateWindow({
	Name = "Matchmaking", 
	Icon = "rendervape/assets/SliderArrow1.png", 
	IconSize = 16
})
...
local TargetInfoToggle = GuiLibrary.ObjectsThatCanBeSaved.GUIWindow.Api.CreateCustomToggle({
	Name = "Target Info",
	Icon = "rendervape/assets/TargetInfoIcon2.png", 
...
local teleportConnection = playersService.LocalPlayer.OnTeleport:Connect(function(): ()
    ...
    if (not teleportedServers) and (not shared.VapeIndependent) then
		...
		local teleportScript = [[
			task.delay(8, function() loadfile('rendervape/core/boot.lua')() end)
		]]
		local script = [[
			local data = 'store'
...
local function initiaterender()
	local profilesdecoded, profiles = pcall(function()
		return httpService:JSONDecode(readfile("rendervape/configuration/"..(bedwars and "6872274481" or game.PlaceId)..".vapeprofiles.txt"))
	end)
	...
	if true then -- don't ask why :)
	    getgenv().rendervapeload = {file = 'universal.lua', step = 3};
		customload(vapeGithubRequest('core/universal.lua'), "universal");
		if bedwars then 
			customload(vapeGithubRequest("games/6872274481.lua"), "6872274481")
		else
	...
	if not shared.VapeSwitchServers then
		if BlatantModeToggle.Enabled then
			pcall(function()
				...
				frame.Frame.Frame.ImageColor3 = Color3.fromRGB(44, 95, 236)
			end)
		end
